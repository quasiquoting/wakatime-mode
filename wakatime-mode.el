...
;; Enable WakaTime for the current buffer by invoking `wakatime-mode'.
;; If you wish to activate it globally, use `global-wakatime-mode'.

;; Set variable `wakatime-api-key' to your API key.
;; Point `wakatime-cli-path' to the absolute path of the CLI script
;; (wakatime-cli.py).

;;; Code:

(defconst wakatime-version "1.0.2"
  "The current version of WakaTime.")

(defconst wakatime-user-agent "emacs-wakatime"
  "The User Agent passed to the WakaTime CLI, \"emacs-wakatime\".")

(defvar wakatime-noprompt nil)

(defvar wakatime-init-started nil
  "Set to t when WakeTime starts initializing.  Default: nil.")

(defvar wakatime-init-finished nil
  "Set to t when WakeTime finishes initializing.  Default: nil.")
...
(defun wakatime-init ()
  ...
  (unless wakatime-init-started
    ...
    (when (or (not wakatime-api-key)
              (string= "" wakatime-api-key))
      (wakatime-prompt-api-key))
    (when (or (not wakatime-cli-path)
              (not (file-exists-p wakatime-cli-path)))
      (wakatime-prompt-cli-path))
    (when (or (not wakatime-python-bin)
              (not (wakatime-python-exists wakatime-python-bin)))
...
(defun wakatime-python-exists (location)
  ...
  (= (condition-case nil
         (call-process location nil nil nil "--version")
       (error 1))
     0))
      ...
      (lambda (process signal)
        (when (memq (process-status process) '(exit signal))
          ...
          (let ((exit-status (process-exit-status process)))
            (unless (or (= 0 exit-status) (= 102 exit-status))
...
(defun wakatime-turn-on (defer)
  "Turn on WakaTime.  If `DEFER' is non-nil, wait \"1 sec\"."
  (if defer
